...
export function LogPanel({ isOpen: controlledOpen, onOpenChange: setControlledOpen }: LogPanelProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 flex flex-col border-t border-border/80 bg-zinc-950 text-zinc-200 shadow-[0_-10px_35px_-5px_rgba(0,0,0,0.5)] animate-page-enter">
      {/* Top Console Header & Close Bar */}
      <div className="flex h-8.5 select-none items-center justify-between px-3 bg-zinc-900 border-b border-zinc-800 text-xs font-medium">
        <div className="flex items-center gap-2.5">
          ...
          {counts.error > 0 && (
            <Badge
              variant="destructive"
              className="h-4 px-1.5 py-0 text-[10px] font-mono bg-rose-500/20 text-rose-400 border border-rose-500/40 gap-1 animate-pulse"
            >
        ...
        <div className="flex items-center gap-1 text-zinc-400">
          <Button
            ...
            title={isExpandedHeight ? "Restore Height" : "Expand Height"}
          >
          ...
          <Button
            variant="ghost"
            size="icon-xs"
            className="size-6 text-zinc-400 hover:text-rose-300 hover:bg-rose-950/40"
