...
function App() {
  ...
  const [showSplash, setShowSplash] = useState(() => {
    try {
      ...
      if (raw) {
        ...
        return parsed.showSplashScreen !== false;
      }
    } catch {
      return true;
    }
  ...
  useEffect(() => {
    ...
    EventsOn("system-tools:status", (statusReport: models.SystemToolsReport) => {
  ...
  return (
    <ThemeProvider defaultTheme={(settings.theme as Theme) || "dark"} storageKey="vite-ui-theme">
      {showSplash && <SplashScreen onComplete={() => setShowSplash(false)} />}

      <div
        id="App"
        className="relative h-screen w-screen bg-background text-foreground flex flex-col overflow-hidden"
      >
          ...
          <div className="w-full max-w-6xl my-auto flex items-center justify-center">
            {report && !report.allInstalled ? (
              ...
              <div
                ...
                className="w-full flex items-center justify-center animate-page-enter"
              >
                {!isConnected ? (
