...
export const SettingsService = {
  async getSettings(): Promise<models.AppSettings> {
    try {
      if (typeof window !== "undefined" && window.go?.app?.App?.GetAppSettings) {
        const settings = await GetAppSettings();
        return new models.AppSettings(settings);
      }
    } catch (err) {
      console.warn(
        "[SettingsService] Failed to load backend settings, falling back to storage/defaults:",
        err
      );
    }
  ...
  async saveSettings(settings: models.AppSettings): Promise<void> {
    ...
    try {
      if (typeof window !== "undefined" && window.go?.app?.App?.SaveAppSettings) {
  ...
  async resetSettings(): Promise<models.AppSettings> {
    try {
      if (typeof window !== "undefined" && window.go?.app?.App?.ResetAppSettings) {
